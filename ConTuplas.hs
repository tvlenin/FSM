...
body xe xa userGroupList userID  = do 
	putStrLn $ show xe
	op <- getLine
	if  head(words(op)) == "print" then			--The sinstaxis must be correct
		printuserg xe xa userID userGroupList 
	{-	Command : #groupadd <nombre del grupo> 
		...
		addUserGroup xe xa userID userGroupList [[(words(op)!!1)],[ show ((read ((((last(userGroupList))!!1)!!0)) :: Integer)+1) ],[],[]]	--Call the function to add the new group
		...
		showAllgroups xe xa userID userGroupList 0
		...
		showAllUsers xe xa userID userGroupList userID userGroupList 
	...
	{- 	Command : # useradd -g primaryGroup [-G secondaryGroup1 secondaryGroup2] userName
		...
		createNewUser xe xa userID [] userGroupList (init(tail(tail(words(op))))) (last(tail(tail(words(op)))))	0
		...
		putStrLn "sobon"
		modifyInformation xe xa userGroupList userID
	...
	-}
		...
		putStrLn "buscando"
		listFiles 0 xe xa userGroupList userID
	else if op == "mv" then do
		putStrLn "Digite la carpte"
		line <- getLine
		moveDirectory line xe xa userGroupList userID
	else do
		putStrLn "Fin"
...
{-------------------------------User---Groups----------------------------------------}
printuserg xe xa userID userGroupList = do
	putStrLn $ (show(userGroupList))
	body xe xa userGroupList userID
{-
	To add the user group with all its information-}
addUserGroup xe xa  userID userGroupList add = do
	checkAndAdd xe xa userID userGroupList 0 add 
	
checkAndAdd xe xa userID userGroupList i add = do
	if( i<= (length(userGroupList)-1)) then do
		if ( (((userGroupList!!i)!!0)!!0) == ((add!!0)!!0)) then do		--this means the user group already exits
			...
			checkAndAdd xe xa userID userGroupList (i+1) add					--check the name of the next user group			
	else do											--The user group does not exits
--		putStrLn $ (show userGroupList)
--		putStrLn $ ("agredando!: "++show(add))
		body xe xa (userGroupList++[add]) userID
		
{-Here is the display of GroupName -> GroupId -> AssociatedUsers -> AssociatedSecondaryUsers
-}
showAllgroups xe xa userID userGroupList i = do
	if( i<= (length(userGroupList)-1) ) then do				--there are still groups to print			
		...
		showAllgroups xe xa userID userGroupList (i+1)
	else do
		body xe xa userGroupList userID	
		
{-This prints all the required information about all the users.
-}
showAllUsers xe xa userID userGroupList users groups  = do
	if ( null users ) then do
		...
		showAllUsers xe xa userID userGroupList (tail(users)) groups 
...
-}
createNewUser xe xa userID userGroupList toCheck args name j = do	
	if ( (length(toCheck) == 0)) then do 							--When this is True, means we can created this user
		if(length(args)==1) then do									--this occurs when there are no secondary groups(-G)
			addNewUserPrimary xe xa userID [] userGroupList 0 (head(args)) [] name
		else do														--if there are secondary groups, I extract the -G term
			addNewUserPrimary xe xa userID [] userGroupList 0 (head(args)) (tail(tail(args))) name
	else do		
		if (not (elem name ((toCheck!!0)!!2))) then do
			createNewUser xe xa userID (userGroupList++[head(toCheck)]) (tail(toCheck)) args name j 
		else do
			putStrLn "The user exists"
			body xe xa userGroupList userID

addNewUserPrimary xe xa userID userGroupList toCheck j primary secondary name = do	
	if ((length(toCheck))==0) then do 							--We checked the whole group names but failed
		...
		if ( (((toCheck!!0)!!0)!!0) ==  primary ) then do	--This means we found the correct group to add the user as primary 			
			addNewUserSecondary xe xa (userID++[[name,  show((read((last(userID))!!1)::Integer)+1)  ]]) [] (userGroupList ++ [ [((toCheck!!0)!!0)] ++ [((toCheck!!0)!!1)] ++ [(((toCheck!!0)!!2)++[name])] ++ [((toCheck!!0)!!3)] ] ++ (tail(toCheck)) ) 0 0 (secondary) name
		else do
			--2 casos:
			...
			if( (j <= (length (((toCheck!!0)!!2))) -1 ) ) then do
				addNewUserPrimary xe xa userID userGroupList toCheck (j+1) primary secondary name -- To compare the next one, inside the same group
			else do
				if(length(toCheck)==1) then do
					addNewUserPrimary xe xa userID (userGroupList++[(head(toCheck))]) [] 0 primary secondary name
				else do 
					addNewUserPrimary xe xa userID (userGroupList++[(head(toCheck))]) (tail(toCheck)) 0 primary secondary name
			  		
addNewUserSecondary xe xa userID userGroupList toCheck i k secondary name = do
	if( null secondary) then do
		--body xe xa (userGroupList++toCheck) userID
		addFiles "d" xe ("home/"++name) xa (userGroupList++toCheck) userID	
		...
		if( i <= ((length(toCheck))-1) ) then do
			if(k <= (length(secondary))-1) then do
				if ( elem (secondary!!k) (((toCheck!!i)!!0)) ) then do	--We found the right place to add it as secondary											
					concatenate xe xa userID userGroupList [] toCheck secondary name 	
				else do
					addNewUserSecondary xe xa userID userGroupList toCheck i (k+1) secondary name
			else do
				addNewUserSecondary xe xa userID userGroupList toCheck (i+1) (0) secondary name
		else do																						--The secondary does not exist
			(addNewUserSecondary xe xa userID (userGroupList++toCheck) [] 0 0 secondary name)
					
concatenate xe xa userID userGroupList answer toCheck secondary name =
	if (null toCheck && null secondary) then do
		addNewUserSecondary xe xa userID userGroupList (answer) 0 0 [] name
	else if ( null toCheck) then do													--done checking, go back to add as secondary
		addNewUserSecondary xe xa userID userGroupList (answer++toCheck) 0 0 secondary name	
	else if(null secondary) then do
		addNewUserSecondary xe xa userID userGroupList (answer++toCheck) 0 0 [] name
		...
		if( (((toCheck!!0)!!0)!!0) == (secondary!!0) ) then do 							--This is the place to add
			if(length(secondary)<=1 && (length(toCheck))<=1) then do
				concatenate xe xa userID userGroupList (((answer++[[[(((toCheck!!0)!!0)!!0)],[(((toCheck!!0)!!1)!!0)],(((toCheck!!0)!!2)),(((toCheck!!0)!!3)++[name])]]))) [] [] name
			else if (length(secondary)<=1) then do
				concatenate xe xa userID userGroupList (((answer++[[[(((toCheck!!0)!!0)!!0)],[(((toCheck!!0)!!1)!!0)],(((toCheck!!0)!!2)),(((toCheck!!0)!!3)++[name])]]))) (tail(toCheck)) [] name 
			else if ((length(toCheck))<=1) then do
				concatenate xe xa userID userGroupList (((answer++[[[(((toCheck!!0)!!0)!!0)],[(((toCheck!!0)!!1)!!0)],(((toCheck!!0)!!2)),(((toCheck!!0)!!3)++[name])]]))) [] (tail(secondary)) name
			else
				concatenate xe xa userID userGroupList (((answer++[[[(((toCheck!!0)!!0)!!0)],[(((toCheck!!0)!!1)!!0)],(((toCheck!!0)!!2)),(((toCheck!!0)!!3)++[name])]]))) (tail(toCheck)) (tail(secondary)) name 
		else do
			concatenate xe xa userID userGroupList (answer++[head(toCheck)]) (tail(toCheck)) secondary name	
		
{-
-}
modifyInformation xe xa userGroupList userID = do
	putStrLn$ "impriendo"
	body xe xa userGroupList userID


findUser xe xa userID userGroupList name j = do
	if ( j<=(length(userID)-1)) then do 
		if(((userID!!j)!!0)==name) then do
			printUserInformation xe xa [] userID name ((userID!!j)!!0) userGroupList 0 0 [] []
		else do
			findUser xe xa userID userGroupList name (j+1)
...
printUserInformation xe xa userGroupList userID name id toCheck i j asPrimary asSecondary= do
	if(null toCheck) then do
		...
		if ( (((toCheck!!0)!!2)!!i) == name ) then do 								--The user is as Primary in this User Group
			printUserInformation xe xa userGroupList userID name id toCheck (i+1) j (((toCheck!!0)!!2)!!i) asSecondary
		else do																		--Let's check the next user set as primary
			printUserInformation xe xa userGroupList userID name id toCheck (i+1) j asPrimary asSecondary
	else if ( j <= (length((toCheck!!0)!!3))-1 ) then do
		if ( (((toCheck!!0)!!3)!!j)==name ) then do 								--The user is as secundary in this User Group
			printUserInformation xe xa userGroupList userID name id toCheck i (j+1) asPrimary (asSecondary++[(((toCheck!!0)!!3)!!j)])
		else do																		--Let's check the next user set as secundary
			printUserInformation xe xa userGroupList userID name id toCheck i (j+1) asPrimary asSecondary
	else do	--This case means we go to the next group
		printUserInformation xe xa (userGroupList++ [head(toCheck)]) userID name id (tail(toCheck)) 0 0 asPrimary asSecondary
		
		
		
		
{------------------------------------------------Functions to manage Files--------------------------------------------------------}
echoFile xe xa doc dir userGroupList userID = do
	if (isNow 0 ("/"++ dir) xe) && (l(xe !! ((getElem 0 dir xe))) == "-") then do
		...
		body ((fhalf 0 xe (getElem 0 dir xe) []) ++ [tuple] ++(shalf ((getElem 0 dir xe)+1) xe (getElem 0 dir xe) []))  xa userGroupList userID
	else do
		putStrLn "Fallo"
...
rmFiles xe dir xa userGroupList userID = do
	...
	if (isNow 0 dir xe) == False then do
		...
		body ((take ((getElem 0 dir xe)) xe )++(drop ((getElem 0 dir xe)+1) xe )) xa userGroupList userID
	else do
		putStrLn "No Se"
...
addFiles mode xe add xa userGroupList userID= do
	now <- getCurrentTime
	...
	if isNow 0 add xe then do
		...
		splitAdd mode mode 0 (splitOn "/" add) xe xa userGroupList userID
	else if (f (head xe )) == "/" then do
		body (xe ++ [(add ,f(head xe) ++ add,getDate now,"time","root","",mode)]) xa userGroupList userID
	else do 
		body (xe ++ [(add, s(head xe)++"/" ++ add,getDate now,"Hora","root","",mode)]) xa userGroupList userID

addFiles2 mode modeT cont dir xe add xa userGroupList userID= do
	now <- getCurrentTime
	if (f (head xe )) == "/" then do
		--putStrLn "d"
		splitAdd mode modeT (cont + 1) dir (xe ++ [(last(splitOn "/" add) ,f(head xe) ++ add,getDate now,"time","root","",mode)]) xa userGroupList userID
	else do
		splitAdd mode modeT (cont + 1) dir (xe ++ [(last(splitOn "/" add) ,f(head xe) ++"/"++ add,getDate now,"time","root","",mode)]) xa userGroupList userID
		
splitAdd mode modeT cont dir xe xa userGroupList userID = do
	if (cont == 0) then do
		putStrLn "s"
		addFiles2 "d" modeT cont dir xe (dir !! 0) xa userGroupList userID
	else if (cont == (length dir)-1) && modeT == "-" then do
		putStrLn "Entra"
		addFiles2 modeT modeT cont dir xe (f(last xe)++"/"++ (dir !! cont)) xa userGroupList userID
	else if (cont < length dir) && (cont /= 0) then do
		putStrLn "5"
		addFiles2 "d" modeT cont dir xe (f(last xe)++"/"++ (dir !! cont)) xa userGroupList userID	
	else
		body xe xa userGroupList userID


listFiles cont  xe xa userGroupList userID = do
	
	if (cont < length xe) then do
		if (f(head xe ) `isInfixOf` s(xe !! cont)) && (f( xe !! 2) /= s(xe !! (cont)))  then do
			putStrLn $ f(xe !! cont)
			listFiles (cont + 1) xe xa userGroupList userID
		else do
			listFiles (cont + 1 ) xe xa userGroupList userID
	else do 
		body xe xa userGroupList userID

moveDirectory dir xe xa userGroupList userID = do
	if dir == ".." then do
		putStrLn "retroceder"
	else if dir == "/" then do
		body ((xe!!1):tail xe) xa userGroupList userID	
	else if (s(head xe) == "/") && (isNow 0 dir xe) && (l(xe !! (getElem 0 dir xe)) == "d") then do
		body ((xe !! getElem 0 dir xe):tail xe) xa userGroupList userID
...
createdDev size dir xe xa userGroupList userID = do 
	if (isNow2 0 dir xe) then do
		putStrLn "SIPP" 
		body xe ((dir,size,dir):xa) userGroupList userID 
	else do
